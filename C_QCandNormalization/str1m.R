...
for(lfc in c(0,0.5,1,1.5)){
  for(pval in c(0.05,0.1)){
    dts<-as.data.frame(summary(decideTests(fit.cont,lfc = lfc,p.value = pval)))
    dts$lfc<-lfc
    dts$fdr<-pval
    dtlist[[paste0("lfc",lfc,"p",pval)]]<-dts
  }
}
...
sink(file = paste0(outname,"/tex/tt.tex"))
kable(tt2,format = "latex",row.names = F,)
sink()
...
saveRDS(tt, file = paste0(datdir,"tt_seqPC1234.rds"))
write.csv(tt, file = paste0(datdir,"tt_seqPC1234.csv"))

# load(paste0(datdir,"rdata.rda"))

eset.cor<-filtNormEset
exprs(eset.cor)<-as.matrix(dat.cor$SeqPC1234)
an<-fData(eset.cor)
an$Gene.description<-gsub("[[:space:]]\\[.+","",an$Gene.description)
fData(eset.cor)<-an

saveRDS(eset.cor,"str1m_spc1234eset.rds")
save.image(file = paste0(datdir,"rdata.rda"))

# load("str1mf2/data/rdata.rda")
